            ...
            <div class="mb-3">
                ...
                <datalist id="datalistOptions">
                    @foreach ($page['karyawan'] as $row)
                        <option @if ($row['id'] == $page['data']['karyawan_id']) {{ 'selected' }} @endif value="{{ $row->id }}">
                            {{ $row->nama }}</option>
                    @endforeach
                </datalist>
            </div>
